...
const gradeOptions = ['六年级', '七年级', '八年级', '九年级', '十年级', '十一年级', '十二年级']
...
function getAssessment(score: number | null) {
  ...
  if (score <= 3) return { title: '刚刚起步', note: '建议搭配一个适配营地，先补齐方法与动手基础。', tone: 'starter' }
  if (score <= 6) return { title: '已有基础', note: '建议用营地快速强化关键技能，再进入项目验证。', tone: 'growing' }
...
async function makePoster(project: Project, camp: Camp | null, competitionNames: string[], assessment: ReturnType<typeof getAssessment>, nickname: string, grade: string): Promise<Poster> {
  ...
  context.fillText(`${nickname} · ${grade}`, canvas.width - margin, 176)
...
export default function App() {
  ...
  const createPoster = async () => {
    ...
    try {
      ...
      setPoster(await makePoster(selectedProject, selectedCamp, names, assessment, nickname, grade))
    } finally {
      setPosterBusy(false)
    }
  }
        ...
        <AnimatePresence mode="wait">
          ...
          {screen === 'project' && (
            <motion.section key="project" className="planner" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
              ...
              <section className={`custom-project${selectedProjectId === 'custom' ? ' is-selected' : ''}`}>
                <div className="custom-project-heading"><span><Plus /></span><div><b>我有自己的课题想法</b><small>DIY 自主命题，不受列表限制</small></div></div>
          ...
          {screen === 'camp' && selectedProject && (
            <motion.section key="camp" className="planner" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
              ...
              <PageHeading eyebrow="第二步 · 选择营地" title="选择项目加速器" copy="高度适配的营地优先展示；其他方向也可自由选择。点名称查看完整介绍。" />
          ...
          {screen === 'result' && selectedProject && (
            <motion.section key="result" className="result-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="student-summary"><UserRound /><div><span>学生信息</span><b>{nickname} · {grade}</b></div></div>
